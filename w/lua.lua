...
MinimizeButton.MouseButton1Click:Connect(function()
    ...
    for _, child in ipairs(Frame:GetChildren()) do
        if child ~= TitleLabel and child ~= CloseButton and child ~= MinimizeButton and child ~= Divider then
            child.Visible = not isMinimized -- Hide/show contents
        end
    end
    -- Adjust Frame size if needed
    if isMinimized then
        Frame.Size = UDim2.new(0, windowWidth, 0, 50) -- Shrink to just show the title
    else
